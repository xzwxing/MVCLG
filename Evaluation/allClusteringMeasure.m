function result = allClusteringMeasure(Y, predY)
...
% 计算聚类纯度（不更改变量名版本）
correnum = 0;
predLidx = unique(predY);          % 确保 predLidx 是唯一聚类标签
pred_classnum = length(predLidx);  % 直接从唯一标签计算聚类数

for ci = 1:pred_classnum
    % 获取当前聚类的真实标签（使用逻辑索引优化）
    cluster_mask = (predY == predLidx(ci));
    incluster = Y(cluster_mask);  % 移除非必要的 find() 函数
    
    % 统计当前聚类中每个真实标签的频次
    if ~isempty(incluster)
        [cnub, ~, idx] = unique(incluster);
        counts = accumarray(idx, 1);  % 高效统计频次
        correnum = correnum + max(counts);
    end
end

Purity = correnum / length(predY);  % 最终纯度计算
...
% ARI
ARI = rand_index1(Y, res, 'adjusted');
